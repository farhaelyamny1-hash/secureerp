...
const AdminUsersOverviewTab = () => {
  ...
  const toggleApproval = async (userId: string, current: boolean) => {
    ...
    toast.success(current ? "تم إلغاء التفعيل" : "تم التفعيل ✅");
    setProfiles((prev) =>
      prev.map((p) => (p.user_id === userId ? { ...p, is_approved: !current } : p))
    );
  };
    ...
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Badge variant="secondary">{profiles.length} مستخدم</Badge>
          {pendingCount > 0 && (
            <Badge variant="destructive">{pendingCount} في انتظار التفعيل</Badge>
          )}
        </div>
      </div>
              ...
              filtered.map((profile) => (
                <TableRow key={profile.id}>
                  ...
                  <TableCell>
                    ...
                    >
                      {profile.is_approved ? (
                        <><UserX className="w-3.5 h-3.5" /> إلغاء</>
                      ) : (
...
export default AdminUsersOverviewTab;
